...
int MyJacobi(VVdouble &Insym, VVdouble &RightEigenV, Vdouble &EigenValues)
{
	...
	for (i = 0; i < MaxNumberOfSweeps; ++i)
	{
		...
		//	LOG(5,<<"sm= "<<sm<<endl);
		if (sm == 0.0)
			return 0; // the program is suppose to return here, after some rounds of i.
		MDOUBLE tresh;
		if (i < 3)
...
void houseHolder(VVdouble &mat, VVdouble &Q)
{
	...
	for (i = n - 1; i > 1; i--)
	{
		...
		/***createing U*******/
		for (r = 0; r < i; r++)
		{ // update vector u with row i the matrix until i; //takes  O(n)
			Q[i][r] = u[r] = mat[i][r];
			Q[r][i] = u[r] / H;
		}
		...
		for (r = 0; r <= i; r++)
		{ // update matrix O(n^2) only part of the matrix
			for (c = 0; c <= i; c++)
...
void QL(Vdouble &d, Vdouble &e, VVdouble &z)
{
	...
	for (l = 0; l < n; l++)
	{
		...
		do
		{
			...
			{
				if (iter++ == 30)
					errorMsg::reportError("Too many iterations in QL");
				g = (d[l + 1] - d[l]) / (2.0 * e[l]);
				r = pythag(g, 1.0);
