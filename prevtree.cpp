...
void tree::TreeNode::removeSon(TreeNode* pSon) {
	static vector<nodeP>::iterator vec_iter;
	vec_iter = remove(_sons.begin(), _sons.end(), pSon);
	_sons.erase(vec_iter,_sons.end()); // pg 1170, primer.
}

void tree::TreeNode::claimSons(){
	int numsons = getNumberOfSons();
...
void tree::clear() {//why do you need this??
	static vector<nodeP> vec;//why not static?
...
void tree::getNeigboursOfNode(vector<nodeP> &vNeighbourVector, const nodeP inNodeP) const {
	...
	for (int i=0; i < inNodeP->getNumberOfSons()/*store it locally just in case*/;++i) {
		vNeighbourVector.push_back(inNodeP->getSon(i));
	}
	if (getRoot() != inNodeP)	
		vNeighbourVector.push_back(inNodeP->father());
	//maybe reserve just in case
}

// get nodePTR from name
// "myNode" is a pointer to the root of the subtree in which we want to find the node "inName" barak- it also searches outside the subtree
tree::nodeP tree::findNodeByName(const string inName, nodeP myNode/* why the f those are not references what a waste of ctors??!*/) const{//radix maybe?
	if (myNode==NULL) myNode=_root;//barak, so if the name does not exist it will continue forever???? and will repeat nodes that have been traversed already? maybe mark the nodes? this is o(n^2) for no reason
	if (myNode->name() == inName) return myNode;
	for (int i=0 ; i < myNode->getNumberOfSons() /*and node not marked*/; i++ ) {
...
//Barak - the most retarded absolutly moranic function I have ever seen. Who programed this shit?!!!!!!!!!
...
void inline tree::getPathBetweenAnyTwoNodes(vector<nodeP> &path, const nodeP node1, const nodeP node2) const {

	path.clear();//why not resize to 0? clear just nullifies the vector for no reason, path should be reserved as well, maybe in the end of this function 
	static vector<nodeP> pathMatrix1;//will be much better to nullify an already existing static vector
	static vector<nodeP> pathMatrix2;//same...

	pathMatrix1.resize(0);
	pathMatrix2.resize(0);

	nodeP nodeup = node1;

	//this is such garbage code!! mark the node with a static long int (array in case of multithreading, 1 for each core/thread)
	//throw one of the paths into path and the other into another static vector for path 2, then copy in bulk to &path
	//remember to count  the sizes to know where to copy to and from
	//if there is a bug set path.size to 0
	...
	while ((tmp1 >= 0) && (tmp2 >= 0)) {//need to look at the algorithm it is possible to cache previous paths
		if (pathMatrix1[tmp1] != pathMatrix2[tmp2]) 
			break;
		tmp1--; 
		tmp2--;
	}
	/// why the Fuck??! don't you push back in the first loop what a waste of 2n 
...
//it might be better to create a direct function no wasteful reverse. why not use a vector of pointers? not hard copies
void tree::getFromLeavesToRoot(vector<nodeP> &vNeighbourVector) const {
	getFromRootToLeaves(vNeighbourVector);
	reverse(vNeighbourVector.begin(),vNeighbourVector.end());//why the F?
}
...
//barak - this implementation should be better but the order is different so i am not sure 
void tree::getFromNodeToLeavesNoRecursion(vector<nodeP> &vec, const nodeP root) const {
	int i = 0;
	int NumOfChilds;
	nodeP iter;

	vec.push_back(root);
	while (i < vec.size()/*grows during this loop with each member added*/) {
		iter = vec[i];
		NumOfChilds = iter->getNumberOfSons();
	
		for (int k=0; k < NumOfChilds; k++) 
			vec.push_back(iter->getSon(k));
...
void tree::getAllHTUs(vector<nodeP> &vec, const nodeP fromHereDown ) const {
	//vec.clear();//resize to 0
	vec.resize(0);
	getAllHTUsPrivate(vec,fromHereDown);
}

//no recursion - stack and no vectors : barak- the greatest comment ever, pushes copies of internal nodes (no leaves or root)
static void tree::getAllHTUsPrivate(vector<nodeP> &vec, const nodeP fromHereDown ) const {
...
//barak - this implementation should be better
void tree::getAllHTUsPrivateNoRecursion(vector<nodeP> &vec, const nodeP root) const {
	int i = 0;
	int NumOfChilds = root->getNumberOfSons();
	nodeP iter;

	//should consider throeing into an inline function repeats 3 times in this file
	for (int k=0; k < NumOfChilds; k++) 
		vec.push_back(root->getSon(k));
	...
	while (i < vec.size()/*grows during this loop with each member added*/) {
		...
		for (int k=0; k < NumOfChilds; k++) {
			nodeP son = iter->getSon(k);
			if (son->getNumberOfSons())//I suspect that this is faster then removing the leaves in the end
				vec.push_back(iter->getSon(k));
		}
...
//getFromNodeToLeavesNoRecursion does the same thing...
...
//shitty recursion and the stack might be too big should write that properly
void tree::getAllLeavesPrivate(vector<nodeP> &vec, const nodeP fromHereDown ) const {
	nodeP son;
	//if (fromHereDown == NULL) return;//barak this condition is useless
	/*if (fromHereDown->isLeaf()) {
		vec.push_back(fromHereDown);
		return;
	}*/

	int num_of_sons = fromHereDown->getNumberOfSons();
	for (int k=0; k < num_of_sons; k++) {
		son = fromHereDown->getSon(k)
		if (fromHereDown->isLeaf()) {
			vec.push_back(fromHereDown)
		}
...
MDOUBLE tree::findLengthBetweenAnyTwoNodes(const nodeP node1, const nodeP node2) const {
	static vector<nodeP> pathMatrix;
	int path_matrix_capacity = pathMatrix.capacity();
	pathMatrix.resize(0);
	MDOUBLE sumOfDistances =0;
	getPathBetweenAnyTwoNodes(pathMatrix, node1, node2); //why not return the length, you can store the partial length in the node itself in this function

	int new_capacity = pathMatrix.capacity();
	if (new_capacity > path_matrix_capacity)
		pathMatrix.reserve(new_capacity);
	//retarded beyond imaginning calculate it in getPathBetweenAnyTwoNodes
	for (int i=0; i < pathMatrix.size()/*wtf*/ ; i++) {
	  // two cases: first, the previous node is closer to the root
	  // than the current one. NOTE: this can not be the case for the
	  // first node in the path
	  if (i>0 /*wtf!!! do once at start*/&&  pathMatrix[i]->father() == pathMatrix[i-1])
	    sumOfDistances += pathMatrix[i]->dis2father();//get...
	  else
...
// similar to above, but for all nodes at once.  O(n^3) or so, but this should not be an issue 
// in any reasonable scenario
// only disTab[i][j] is filled. disTab[j][i] remains zero.
vsoid tree::getTreeDistanceTableAndNames(VVdouble& disTab, vector <string>& vNames) const {
  static vector<nodeP> nodepV;
...
void tree::rootAt(const nodeP p_iNewRoot) {
	if (_root == p_iNewRoot) 
		return;
	statsic vector<nodeP> pathMatrix(100);
	size_t old_cap = pathMatrix.capacity();
	pathMatrix.resize(0);
	getPathBetweenAnyTwoNodes(pathMatrix, _root, p_iNewRoot);
	size_t new_cap = pathMatrix.capacity();

	if (new_cap > old_cap)
		pathMatrix.reserve(new_cap);
...
void create_names_to_non_leaves_recursive_private(node_p subtreeRoot, in num_of_childs,int num) {
	int num_of_children = subtreeRoot.getNumberOfSons();
	int son_num_of_childern;

	subtreeRoot->setName((string)"N" + int2string(num));
	for (int i = 0; i< num_of_children;++i)
	{
		if (subtreeRootsubtreeRoot.getNumberOfSons())
		create_names_to_non_leaves_recursive_private(,num+num_of_children);
	}
}

//keep the factor local and multiply by it in dis2father(), it will be much faster 
