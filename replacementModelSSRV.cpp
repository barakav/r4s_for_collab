...
replacementModelSSRV::replacementModelSSRV(const distribution *dist, const replacementModel *baseRM, MDOUBLE rateOfRate /*= 1 */) : _dist(dist->clone()),
																																	_baseRM(baseRM->clone()),
																																	_rateOfRate(rateOfRate)
{
	if (_rateOfRate < EPSILON)
		_rateOfRate = EPSILON; // ??? Temporary - to overcome a bug in QL algorithm, when _rateOfRate == 0
...
replacementModelSSRV::replacementModelSSRV(const replacementModelSSRV &other) : _dist(other._dist->clone()),
																				_baseRM(other._baseRM->clone()),
																				_rateOfRate(other._rateOfRate),
																				_q2pt(other._q2pt),
																				_freq(other._freq),
...
{
	_Q.clear(); // realy? maybe just memset(,size,0) after resize on general why doing it like this at all? allocate memory at a single bulk
	int size = alphabetSize(); //doesnt it stay the same? why release and alllocate at all?
	_Q.resize(size);
	for (int z = 0; z < _Q.size(); ++z)
	...
	for (int i = 0; i < _BaseRM_alphabetSize; ++i) // multitheareading/ noraml two dimentional array and one loop
	{
		for (int j = 0; j < _BaseRM_alphabetSize; ++j)
		{
			for (int z = 0; z < mulFactor; ++z)
			{
				for (int w = 0; w < mulFactor; ++w)
				{
					...
					{
						if (z == w)
							_Q[z * _BaseRM_alphabetSize + i][z * _BaseRM_alphabetSize + j] = _dist->rates(z) * _baseRM->dPij_dt(i, j, 0);
					}
					else
					{
						if (z != w)
