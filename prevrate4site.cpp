...
void rate4site::getStartingSequenceData(){
	...
	ifstream in(_options->seqfile.c_str());//mmap?
	...
	if ((_options->treefile=="") || bCalcDistanceTable) {
		...
		switch (_options->treeSearchAlg){
			...
			case (rate4siteOptions::njJCOLD):
				pDm = new jcDistanceOLD(_options->alphabet_size);//change this retarded name to something clear or erase this shit and repkace with new
															     //it might be a better idea to erase the empty parts aktogether and use this then return them to their rightfull place
				giveDistanceTable(pDm, _sc,disTab, vNames);
